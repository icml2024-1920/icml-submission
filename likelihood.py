...
def get_likelihood_fn(sde, model, inverse_scaler, hutchinson_type='Rademacher',
  ...
  def likelihood_fn(prng, pstate, data):
    ...
    def ode_func(t, x):
      sample = utils.from_flattened_numpy(x[:-shape[0] * shape[1]], shape)
      vec_t = jnp.ones((sample.shape[0], sample.shape[1])) * t
      drift = utils.to_flattened_numpy(p_drift_fn(pstate, sample, vec_t))
      logp_grad = utils.to_flattened_numpy(p_div_fn(pstate, sample, vec_t, epsilon))
      return np.concatenate([drift, logp_grad], axis=0)

    init = jnp.concatenate([utils.to_flattened_numpy(data), np.zeros((shape[0] * shape[1],))], axis=0)
    solution = integrate.solve_ivp(ode_func, (eps, sde.T), init, rtol=rtol, atol=atol, method=method)
    nfe = solution.nfev
    zp = jnp.asarray(solution.y[:, -1])
    z = utils.from_flattened_numpy(zp[:-shape[0] * shape[1]], shape)
