...
from utils import get_temb_head
...
class NCSNpp(nn.Module):
  ...
  @nn.compact
  def __call__(self, x, time_cond, index_cond, train=True):
    """
      x:          Input image of shape  (B, H, W, C)
      time_cond:  time condition, shape (B,)
      index_cond: Head selection, shape (B,) (int)
    """
    # config parsing
    config = self.config
    act = get_act(config)
    ...
    reflow_t = config.training.reflow_t
    ...
    if conditional:
      temb = nn.Dense(nf * 4 * reflow_t, kernel_init=default_initializer())(temb)
      temb = get_temb_head(temb, index_cond, reflow_t)
      temb = nn.Dense(nf * 4 * reflow_t, kernel_init=default_initializer())(act(temb))
      temb = get_temb_head(temb, index_cond, reflow_t)
    else:
      temb = None
